...
bool found=0;
int indexnew;
void modifiedtry1 (struct Env * curenv)
{
	uint32 pointer = curenv->page_last_WS_index;
	uint32 szofenv=env_page_ws_get_size(curenv);
	uint32 per;
	int tempsz=0;
	while (tempsz<szofenv)
	{
		if (pointer==szofenv) // all pages working set
			pointer=0;

		uint32 tempva=env_page_ws_get_virtual_address(curenv,pointer);
		per=pt_get_page_permissions(curenv,tempva);
		if (!(per&PERM_USED) && !(per&PERM_MODIFIED))
		{
			victim=tempva;
			found=1;
			indexnew = pointer;
			curenv->page_last_WS_index=(curenv->page_last_WS_index+1)%szofenv;
			break;
		}
		pointer++;
		tempsz++;
...
void modifiedtry2 (struct Env * curenv)
{
	uint32 pointer = curenv->page_last_WS_index;
	uint32 szofenv=env_page_ws_get_size(curenv);
	uint32 per;
	...
	while (tempsz<szofenv)
	{
		...
		pt_set_page_permissions(curenv,tempva,0,PERM_USED);
		pointer++;
		tempsz++;
	}
...
void placement (struct Env * curenv, uint32 fault_va)
{
	...
	/*cprintf("before\n");
			...
			if((page_permissions&PERM_BUFFERED)>0) // the page is already BUFFERED ^_^
			{

				pt_set_page_permissions(curenv , fault_va , PERM_PRESENT , PERM_BUFFERED);
				struct Frame_Info * f = get_frame_info(curenv->env_page_directory,(void *)fault_va,&ptr_table);
				f->isBuffered=0;
				if ((page_permissions&PERM_MODIFIED)>0) // the page is already MODIFIED ^_^
				{
					bufferlist_remove_page(&modified_frame_list,f);

				}
				else // the page is not MODIFIED ^_^
				{
					bufferlist_remove_page(&free_frame_list,f);
				}
			}
			else // the page is not BUFFERED
			{
				struct Frame_Info *ptr_new_frame = NULL;
				int ret = allocate_frame(&ptr_new_frame);
				map_frame(curenv->env_page_directory,ptr_new_frame,(void*)fault_va,PERM_USER|PERM_WRITEABLE|PERM_PRESENT);
		...
		/*	if (curenv->page_last_WS_index+1<curenv->page_WS_max_size)
			...
			for(int i=0;i< curenv->page_WS_max_size;i++){
				int is_empty = env_page_ws_is_entry_empty(curenv,i);
				if(is_empty){
...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va)
{
 ...
 //			env_page_ws_print(curenv);
			//cprintf("here\n");
			uint32 *ptr_table = NULL;
			// change statue of the frame that will be replace
			struct Frame_Info *f = get_frame_info(curenv->env_page_directory, (void *) victim , &ptr_table);
			f->isBuffered=1;
			f->environment=curenv;
			f->va=victim;
			/*ptr_table[PTX(victim)] |= (PERM_BUFFERED) ;
			ptr_table[PTX(victim)] &= ~(PERM_PRESENT) ;*/
			pt_set_page_permissions(curenv , victim,PERM_BUFFERED,PERM_PRESENT);
			uint32 per = pt_get_page_permissions(curenv , victim);
		   	env_page_ws_clear_entry(curenv,indexnew);
			// checking to know if we need to write on the page file or not ^_^
			if ((per&PERM_MODIFIED)==0)
			{
			...
			else
			{
				...
				if(LIST_SIZE(&modified_frame_list)>=getModifiedBufferLength())
					{
						struct Frame_Info *it;
						LIST_FOREACH(it,&modified_frame_list)
								{
									pf_update_env_page(it->environment,(void*)it->va,it);
									bufferlist_remove_page(&modified_frame_list,it);
									bufferList_add_page(&free_frame_list,it);
									pt_set_page_permissions(it->environment,it->va,0,PERM_MODIFIED);
								}
					}

			}

		// we now ready to make new placement for our fault_va
		  placement(curenv,fault_va);
   found =0 ;
		}
