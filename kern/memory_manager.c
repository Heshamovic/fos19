...
{
	uint32  va=(uint32)kmalloc(1024*4);
	uint32 tempphy=kheap_physical_address(va);
	ptr_page_directory[PDX(virtual_address)]=CONSTRUCT_ENTRY(tempphy,PERM_USER|PERM_WRITEABLE|PERM_PRESENT);
	memset((void *)va , 0 , PAGE_SIZE);
	tlb_invalidate(ptr_page_directory,(void *)va);
	return (void*)va;
}
