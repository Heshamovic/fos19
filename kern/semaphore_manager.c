...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	struct Semaphore *sptr;
	int ch = get_semaphore_object_ID(ownerEnvID, semaphoreName);
	if (ch == E_SEMAPHORE_NOT_EXISTS)
	{
		int x = allocate_semaphore_object(&sptr);
		strcpy(sptr->name, semaphoreName);
		sptr->ownerID = ownerEnvID;
		sptr->value = initialValue;
		return x;
	}
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int i = get_semaphore_object_ID(ownerEnvID, semaphoreName);
	if (i == E_SEMAPHORE_NOT_EXISTS)
	{
		fos_scheduler();
		return;
	}
	...
	if (semaphores[i].value < 0)
	{
		...
		enqueue(&(semaphores[i].env_queue), env);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if (semaphores[i].value <= 0)
	{
		struct Env *env = dequeue(&(semaphores[i].env_queue));
